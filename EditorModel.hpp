...
public:  //added functions
    ...
    void previousSpot(unsigned int& ln, unsigned int& cn);

    void deleteLine();

    std::string tempHolder();

    void addBackIn(unsigned int& lnum, std::string& str);
...
/*
Every line number needs to keep track of how many 
    characters have been added.
    call line(), 
    if lineNumber == line() -> moveCursorLeft() cannot go pass line().end()
    if current line is longer than the desired line destination
        and the position of the cursor is further
            moveCursorUp() and moveCursorDown() will go to line().end()
            else it will simply just add/subtract current line number
*/
